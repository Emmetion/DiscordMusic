...
class MyClient(discord.Client):
    ...
    async def play_audio(self, message, youtube_url):
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            try:
                embed = discord.Embed(title='Searching...', description=f'Query: {youtube_url}', color=discord.Color.yellow())
    ...
    async def display_queue(self, message):
            if len(self.__queue) > 0:
                ...
                for (tempMessage, youtube_url) in self.__queue:
                    if (i == 1):
                        embed.add_field(name=f"Next Up", value=f"**Added by:** {tempMessage.author}\n**URL:** {youtube_url}", inline=False)
                    else:
                        embed.add_field(name=f"Position {i}", value=f"**Added by:** {tempMessage.author}\n**URL:** {youtube_url}", inline=False)
                    i += 1
